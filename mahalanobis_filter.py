...
class MahalanobisFilter(nnx.Module):
    def __init__(
            self, 
            learn_rate: jax.Array, 
            threshold: jax.Array, 
            input_size: jax.Array
        ):
    ...
    def update_distribution(self, x: jax.Array):
        d = x - self.mu
        self.P = (
    ...
    def get_M_distance(self, x: jax.Array) -> jax.Array:
        d = x - self.mu
        return jnp.sqrt(jnp.transpose(d) * self.P * d)
    
    def filter(self, x: jax.Array) -> jax.Array:
