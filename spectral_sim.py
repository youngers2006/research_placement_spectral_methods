...
class HyperElasticRVE():
    def __init__(self, *, order=(4,4,4), nq=50, barrier=1e-6, alpha=1e-6,
        ...
        self.coeffs = jnp.zeros((3, self.NB))
    ...
    def face_param_grid(self, N: int):
        ...
        S, T = jnp.meshgrid(g, g, indexing="ij")      # (N,N)
        st = jnp.stack([S.ravel(), T.ravel()], axis=1)  # (N^2,2)
        return S, T, st
    ...
    def face_points(self, axis: str, side: str, N: int = None):
        ...
        pts_face = jnp.stack([Xf.ravel(), Yf.ravel(), Zf.ravel()], axis=1)  # (N^2,3)
    ...
    def _face_normal(axis: str, side: str):
        ...
        return jnp.array([0.0, 0.0, 1.0])  # axis == "z"
    ...
    def legendre_derivs_from_vals2(vals, xhat):
        ...
        derivs = jnp.zeros_like(vals)
        # n=0 -> 0
        if m >= 2:
            n_idx = jnp.arange(1, m)
            # broadcast-safe
    ...
    def legendre_derivs_from_vals(vals, xhat, eps=1e-12):
        ...
        if m >= 2:
            ...
            if jnp.any(at_m1):
                ...
                sgn = jnp.where((n_idx % 2) == 1, 1.0, -1.0)
    ...
    # Tensor-product basis at points in [0,1]^3
    @staticmethod
    def map_to_hat(p):
        return 2.0 * p - 1.0
    ...
    def neo_hookean_energy_core(coeffs, B, dBdx, dBdy, dBdz, E, nu, w, barrier, alpha):
        ...
        I = jnp.eye(3)
        F = jnp.stack([
            jnp.stack([I[0,0] + du_dx[:,0], du_dy[:,0], du_dz[:,0]], axis=1),
            jnp.stack([du_dx[:,1], I[1,1] + du_dy[:,1], du_dz[:,1]], axis=1),
            jnp.stack([du_dx[:,2], du_dy[:,2], I[2,2] + du_dz[:,2]], axis=1)
        ], axis=1)
    
        # after you build F
        J = jnp.linalg.det(F)
        J_safe = jnp.maximum(J, 1e-12)          # or use your barrier epsilon
        
        C  = jnp.einsum('...ji,...jk->...ik', F, F)
        I1 = jnp.trace(C, axis1=1, axis2=2)
        ...
        W_bar = jnp.where(J > barrier, -alpha * jnp.log(J),
    ...
    def unit_cube_quadrature(self):
        ...
        # nq = nq
        grid = (jnp.arange(self.nq) + 0.5)/self.nq
        X, Y, Z = jnp.meshgrid(grid, grid, grid, indexing='ij')
        self.pts = jnp.stack([X.ravel(), Y.ravel(), Z.ravel()], axis=1)
        # print(X.ravel().shape)
        # print(nq)
        self.w = jnp.full((self.pts.shape[0],), (1.0/self.nq)**3)
    
    def gyroid_E(self, points):
        # use instance params (no kwargs)
        x, y, z = points[:,0], points[:,1], points[:,2]
        g = (jnp.sin(2*jnp.pi*x)*jnp.cos(2*jnp.pi*y)
           + jnp.sin(2*jnp.pi*y)*jnp.cos(2*jnp.pi*z)
           + jnp.sin(2*jnp.pi*z)*jnp.cos(2*jnp.pi*x))
        inside = (jnp.abs(g) < self.wall_thickness)
        return jnp.where(inside, self.E_in, self.E_out)
    ...
    def cross_E(self, points):
        ...
        Typical useful values: ~0.01_0.15 (much smaller than for gyroid).
        """
        x, y, z = points[:, 0], points[:, 1], points[:, 2]
    ...
    def material_E(self, points):
        ...
        if self.geom == "cross":
            ...
            return self.E_in * jnp.ones(points.shape[0])
        elif self.geom == "void":
            return self.E_out * jnp.ones(points.shape[0])
        else:
            return self.gyroid_E(points)  
    ...
    def filtered_quadrature(self):
        ...
        """
        inside_np = self.E > 0.5*(self.E_in + self.E_out)
    ...
    def build_basis_and_grads(self,points):
        ...
        self.B = jnp.stack(cols, axis=1)
        self.dBdx = jnp.stack(dx_cols, axis=1)
        self.dBdy = jnp.stack(dy_cols, axis=1)
        self.dBdz = jnp.stack(dz_cols, axis=1)
        
    def prepare_jit(self):
        # JIT the core energy; other args are treated as constants at call time
        self.Energy_fn = jax.jit(lambda A: self.neo_hookean_energy_core(
            A, 
            self.B, 
            self.dBdx, 
            self.dBdy,
        ...
        self.dEnergy_dA_fn = jax.jit(
            grad(lambda A: self.neo_hookean_energy_core(
                    A, 
                    self.B, 
                    self.dBdx, 
                    self.dBdy, 
